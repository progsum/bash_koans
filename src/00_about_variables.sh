...
test_setting_the_variable() {
  local variable=1              # @progsum : local variable... use this
...
test_global_variables_from_functions() {
  assertEqual $THIS_VARIABLE_IS_GLOBAL_FROM_A_FUNCTION "42"  #@progsum : type does not matter
}
...
test_local_variables() {
  assertEqual $THIS_VARIABLE_IS_LOCAL $THIS_VARIABLE_IS_LOCAL
}
...
test_only_exported_variables_are_accessible_by_another_process() {
  ...
  assertEqual "$(support/variable_check)" 43    # @progsum : check this!!
}
